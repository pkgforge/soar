...
impl Queryable<packages::SqlType, Sqlite> for Package {
    ...
    fn build(row: Self::Row) -> diesel::deserialize::Result<Self> {
        Ok(Self {
            ...
            tags: json_vec!(row.22),
            categories: json_vec!(row.23),
            build_id: row.24,
            build_date: row.25,
            build_action: row.26,
            build_script: row.27,
            build_log: row.28,
            provides: json_vec!(row.29),
            snapshots: json_vec!(row.30),
            replaces: json_vec!(row.31),
            soar_syms: row.32,
            desktop_integration: row.33,
            portable: row.34,
            recurse_provides: row.35,
        })
    }
}
