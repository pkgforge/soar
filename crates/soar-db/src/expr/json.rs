...
/// ```
///
/// Under the hood, it uses SQLite’s [`json_each`](https://www.sqlite.org/json1.html#jeach)
...
impl<T> JsonArrayContains<T> {
    ...
    /// ```
    pub fn like(self, pattern: impl Into<String>) -> JsonArrayOp {
        JsonArrayOp {
            column: self.column,
            field: self.field,
            op: "LIKE",
            value: Value::Text(format!("%{}%", pattern.into())),
        }
    }
}
