...
fn display_diff(diff: &ApplyDiff, prune: bool) {
    ...
    if !diff.to_install.is_empty() || !diff.to_update.is_empty() || (prune && !diff.to_remove.is_empty()) {
        ...
        for (_resolved, target) in &diff.to_install {
            ...
            builder.push_record([
                ...
                format!("{}#{}", Colored(Blue, &pkg.pkg_name), Colored(Cyan, &pkg.pkg_id)),
        ...
        for (_resolved, target) in &diff.to_update {
            let pkg = &target.package;
            let old_version = target.existing_install.as_ref().map_or("?".to_string(), |e| e.version.clone());
            builder.push_record([
                format!("{}", Colored(Yellow, icon_or("~", "~"))),
                format!("{}#{}", Colored(Blue, &pkg.pkg_name), Colored(Cyan, &pkg.pkg_id)),
                format!("{} -> {}", Colored(Red, &old_version), Colored(Green, &pkg.version)),
                format!("{}", Colored(Magenta, &pkg.repo_name)),
            ]);
        }
